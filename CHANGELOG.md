...
- Removed `jax_fdm/visualization/notebooks/register.py`: the compas 1.x `Artist.register` factory it targeted no longer exists. Scene registration now lives in `jax_fdm/visualization/register.py` and the per-backend `scene_objects.py` modules; the mesh scene objects pin the native mesh scene object via `sceneobject_type` to avoid recursing into the registered force density type.
