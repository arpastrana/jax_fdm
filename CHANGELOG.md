...
- Added typing-only mixin classes in `datastructures/types.py` (`FDMeshType`, `FDNetworkType`, `FDDatastructureType`), inherited as the first base of `FDMesh`, `FDNetwork` and `FDDatastructure`. The mixins are empty at runtime — their bodies exist only under `TYPE_CHECKING`, so the MRO passes straight through to the COMPAS implementations — and re-declare, for static checkers, what COMPAS's python-2 type comments misreport: the inherited constructors (`from_meshgrid`, `from_obj`, `from_json`, `from_vertices_and_faces`, `from_polyhedron`, `from_lines`, `from_nodes_and_edges`) and chainables (`copy`, `transformed`, `subdivided`) return the FD subclass, not base `Mesh`/`Data` (they construct through `cls` at runtime), and the getter/setter dual accessors user scripts call (`vertices`/`nodes`, `vertices_where`/`nodes_where`, `vertex_attribute`/`node_attribute` and friends, `edges`, `edge_attribute`, coordinate getters) have mode-split `@overload` signatures instead of one broad inferred union. Downstream, `FDMesh.from_meshgrid(...)` now types as `FDMesh`, so FD-specific method calls and `fdm(mesh)` type-check; in-package, the mixin declarations retire all ten `FDDatastructure` attribute-access warnings and about thirty `# pyright: ignore` suppressions across the datastructures, parameters, structures, and visualization modules. `OptimizationRecorder.from_json` gets the same `Self`-returning treatment. The two runtime `edges()` passthrough overrides on `FDMesh` and `FDNetwork`, which existed only to re-type `super().edges(data)`, are deleted; the FD-owned getter/setter duals (`vertices_supports`/`vertices_fixed`, `nodes_supports`/`nodes_fixed`/`nodes_anchors`) gained real overload pairs so their getter mode types as a plain iterator instead of `Iterator | None`.

### Changed

- Changed the pyright scope to also check `examples/` — the downstream-user perspective the typing mixins exist for — and restored `reportAttributeAccessIssue` to its default error severity, since the dynamic-attribute warnings it silenced are gone. The examples were brought to zero diagnostics with behavior-neutral edits: `recorder is not None` guards instead of re-testing the `record` flag, plain point-pair targets instead of COMPAS `Line` objects, and in `dome.py` a rename of the shadowing `optimizer = optimizer()` rebind plus a guard reorder that previously read `recorder` before its first binding.
...
- Changed the non-overload typing stubs in `datastructures/types.py` and on `OptimizationRecorder.from_json` to raise `NotImplementedError` instead of `...`: pylint's inference reads a `...`-bodied stub as returning `None` and flags call sites with E1111/E1133. Equivalent for pyright; the stubs still never execute.
