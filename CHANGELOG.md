...
- Added typing-only mixin classes in `datastructures/types.py` (`FDMeshType`, `FDNetworkType`, `FDDatastructureType`), inherited as the first base of `FDMesh`, `FDNetwork` and `FDDatastructure`. The mixins leave no trace at runtime: their names are bound to PEP 560 placeholders whose `__mro_entries__` erases them from the class bases, so the runtime MRO is exactly as if they were never listed — necessary because COMPAS's JSON serializer walks the MRO calling `__clstype__()` on every base, which any foreign class would crash (regression-tested in `tests/test_datastructure_types.py` along with the JSON round-trip it protects). For static checkers they re-declare what COMPAS's python-2 type comments misreport: the inherited constructors (`from_meshgrid`, `from_obj`, `from_json`, `from_vertices_and_faces`, `from_polyhedron`, `from_lines`, `from_nodes_and_edges`) and chainables (`copy`, `transformed`, `subdivided`) return the FD subclass, not base `Mesh`/`Data` (they construct through `cls` at runtime; `copy(cls=Other)` returns an `Other` and is typed so through an overload pair, and the `from_json`/`from_obj` file paths accept path-like objects as the runtime does), and the getter/setter dual accessors user scripts call (`vertices`/`nodes`, `vertices_where`/`nodes_where`, `vertex_attribute`/`node_attribute` and friends, `edges`, `edge_attribute`, coordinate getters) have mode-split `@overload` signatures instead of one broad inferred union. Downstream, `FDMesh.from_meshgrid(...)` now types as `FDMesh`, so FD-specific method calls and `fdm(mesh)` type-check; in-package, the mixin declarations retire all ten `FDDatastructure` attribute-access warnings and about thirty `# pyright: ignore` suppressions across the datastructures, parameters, structures, and visualization modules. `OptimizationRecorder.from_json` gets the same `Self`-returning treatment. Non-overload stub bodies raise `NotImplementedError` rather than `...`, since pylint's inference reads a `...`-bodied stub as returning `None` and flags call sites with E1111/E1133; the stubs never execute either way. The two runtime `edges()` passthrough overrides on `FDMesh` and `FDNetwork`, which existed only to re-type `super().edges(data)`, are deleted; the FD-owned getter/setter duals (`vertices_supports`/`vertices_fixed`, `nodes_supports`/`nodes_fixed`/`nodes_anchors`) gained real overload pairs so their getter mode types as a plain iterator instead of `Iterator | None`.
...
- Changed the modules that feed the dir()-based package `__all__`s to declare explicit `__all__` lists, so the star-exported package namespaces carry only the public API. Typing machinery (`Sequence`, `Any`, `TypeVar`, the `TargetLike` and `AnyFDDatastructure` aliases, the typing mixin classes) and incidental imports (`jnp`, `vmap`, geometry helpers, COMPAS classes) no longer appear in `jax_fdm.datastructures`, `equilibrium`, `goals`, `losses`, `constraints` and `optimization`. `EdgeForceDensityParameter` also stops being importable from `jax_fdm.optimization` — it only ever appeared there because the optimizer module imports it for its default parameter — so import it from its home package, `jax_fdm.parameters`; the examples, tests and documentation guide are repointed accordingly. This is a breaking change for imports of these incidental names from package namespaces instead of their home modules.
