...
- Repointed the `viz` optional-dependency extra from the discontinued `compas_view2==0.7.0` (COMPAS 1.x only, which made `pip install jax_fdm[viz]` unresolvable against the new `compas>=2.15` pin) to its COMPAS 2.x successor `compas_viewer>=1.0`. This makes the extra installable again; porting the viewer code itself off `compas_view2.app.App` onto `compas_viewer` remains future work, so the 3D viewer still degrades to the `null_viewer` stub at runtime until then.
