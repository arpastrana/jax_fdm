...
- Added `FDMeshArtist` and its `compas_viewer` backend `FDMeshViewerArtist`, so an `FDMesh` renders directly with its force density metadata (edge colors and widths, support colors, load and reaction vectors) plus a shaded surface for its faces. `viewer.add(mesh)` now routes an `FDMesh` through the mesh artist, removing the `FDNetwork.from_mesh(mesh)` detour. Updated the `monkey_saddle`, `monkey_saddle_constraints` and `pillow` examples accordingly.
- Ported the notebook backend to compas_notebook 2.x (`>=0.11`). `NotebookViewer` now subclasses `compas_notebook.viewer.Viewer`, dispatches an `FDNetwork`/`FDMesh` by type through the new notebook artists, and folds the `width`/`height`/`show_grid`/`viewport`/`camera_position`/`camera_target` kwargs into a `compas_notebook.config.Config`. Added `FDMeshNotebookArtist` on a shared `FDDatastructureNotebookArtist` base. The notebook artists batch each element category into a single mesh with per-face colors to keep the Jupyter widget count low; rendering is draw-once (no scene tree, no animation loop).
- Registered `FDNetwork` and `FDMesh` with the COMPAS 2.x scene system, so a native COMPAS scene — a bare `compas_viewer.Viewer` or a compas_notebook scene — renders them through the jax_fdm artists (e.g. `scene.add(fdmesh, edgecolor="force")`). Registration runs automatically at `import jax_fdm.visualization`, per installed backend.
- Extracted an `FDShapeArtist` mixin with the shape-producing draw hooks (edges as cylinders, points as spheres, loads and reactions as arrows), shared by the viewer and notebook backends.
...
- Committed the sources of the three Colab notebooks linked in the README to a new `notebooks/` folder, migrated to the COMPAS 2.x API (the published versions still targeted compas 1.x: `Polyline.divide_polyline`, `compas.utilities.pairwise`, the 2D `Plotter`, `as_wireframe=True`). The arch notebook renders through `NotebookViewer(viewport="top")` instead of the discontinued plotter. The vault notebook's `butt*.json` inputs were lost from `main` (they now 404); its network files turned out to be identical to the committed `vault.json`/`vault0.json`, so it downloads those instead and meshes the target network with `Mesh.from_lines` for the surface display, dropping the third data file altogether. `pip install` in the notebooks also pulls `compas_notebook` explicitly, since Colab does not install jax_fdm's `viz` extra.

### Changed

- Extracted a shared `FDDatastructureArtist` base (and `FDDatastructureViewerArtist` for the viewer backend) from `FDNetworkArtist`, which keeps its name and its `network`/`nodes` aliases. `viewer.add` now dispatches purely by type, with no `as_wireframe`/`as_mesh` flags: to draw a force density datastructure as plain geometry, convert it first, e.g. `viewer.add(mesh.copy(cls=Mesh))` or `viewer.add(network.copy(cls=Network))`. A converted network is displayed as "Network" rather than "Graph" (COMPAS 2.x aliases `Network` to `Graph`); an explicit `name=` still wins.
- Bumped the COMPAS pin from `compas<2.0` to `compas>=2.15,<3.0` and migrated the codebase to the COMPAS 2.x API: `OptimizationRecorder` now serializes via `__data__`/`__from_data__`, the engine's update helpers use `index_node()`/`index_vertex()`/`index_edge()` instead of the removed `index_key()`/`index_uv()` (`FDMesh` grows an `index_edge()` to match `Graph`), and `mesh.edge_faces((u, v))` takes an edge tuple. The moved imports and renamed calls in the test suite were updated, and the committed JSON fixtures re-serialized into the 2.x format.
...
- Decoupled the base visualization artist from the removed COMPAS artist machinery. `FDNetworkArtist` no longer subclasses `compas.artists.NetworkArtist` (gone in COMPAS 2.x, which broke `import jax_fdm.visualization`); it is now a backend-neutral base class that always imports, and concrete backends implement the `draw_*` methods. `remap_values` now imports from `compas.itertools`.
...
- Ported the animation examples to `compas_viewer`, following its dynamic-drawing pattern: the frame callback goes through `@viewer.on(interval, frames)` (the old `timeout`/`record` kwargs are gone), mutates the datastructures in place and refreshes each scene object with `update(update_data=True)`. `animation_mesh.py` now animates the `FDMesh` directly through its viewer artist. `OptimizationRecorder` grew a `__len__` so `len(recorder)` reports the number of recorded frames.
- `FDMeshViewerArtist.update()` now also refreshes the shaded "Faces" surface, so an animated mesh redraws its faces alongside the wireframe.
- Repointed the `viz` extra from the discontinued `compas_view2==0.7.0` to `compas_viewer>=1.0`, making `pip install jax_fdm[viz]` resolvable again. The extra also grew `compas_notebook>=0.11` with the notebook port.
- Ported the 3D `Viewer` off the discontinued `compas_view2.app.App` onto `compas_viewer`. The wrapper subclasses `compas_viewer.Viewer` and only overrides `add` to route the force density datastructures through their viewer artists; the `width`/`height`/`show_grid` kwargs fold into a `compas_viewer.config.Config`. The custom `Arrow` shape moved to `jax_fdm/visualization/shapes.py` so every backend can share it, and the backend, `conftest` and example wiring was repointed (`viewer.view.camera` became `viewer.renderer.camera`, `rotation[2]` became `rotation.z`).
...
- Updated the README and the bug-report issue template for the migration: dropped the COMPAS 1.x conda install step, listed Python 3.12 as supported, and noted Matplotlib as a core dependency.
...
- Removed the `examples/arch/arch_plotter.py` example. It relied on `compas_plotters`, which has no COMPAS 2.x release; `examples/arch/arch.py` covers the same case.
- Removed `jax_fdm/visualization/notebooks/register.py`: the compas 1.x `Artist.register` factory it targeted no longer exists. Scene registration now lives in `jax_fdm/visualization/scene.py` and the per-backend `scene.py` modules; the mesh artists pin the native mesh scene object via `sceneobject_type` to avoid recursing into the registered force density adapter.
