...
- Dropped the `compas_view2` dependency from `examples/pringle/pringle_temporal_horizontal.py`: the tangent-direction arrows are now drawn as a `compas.geometry.Line` with a `Point` at the tip (colored by tangent angle as before), and the unused normal arrows were removed. This clears the last `compas_view2`/`compas_singular`/`compas_quad` import from `examples/`.
- Reworked `examples/pringle/pringle_temporal_horizontal.py` to run its 3D courses on an `FDMesh` so the (now `VertexTangentAngleGoal`) tangent goal has faces: each course's quad strip is recorded as explicit faces, the network snapshot is wrapped as a mesh for the constrained solve, and the solved coordinates/force densities are copied back onto the network. `qmax` is now slightly negative (`-0.001`) so spine edges keep a compressive force density instead of reaching exactly zero, which the FD solver rejects. This is the first time the example runs end to end.
...
- Fixed `VertexNormalAngleGoal` (and thus `VertexTangentAngleGoal`) chasing the wrong angle branch. The prediction took the angle from the *signed* cosine of the vertex normal against the reference vector, so it depended on face winding: a mesh whose faces wound the averaged vertex normal downward reported `pi - theta` instead of `theta`. On the pringle example every vertex normal pointed down, so the tangent goal (`pi/2 - angle_normal`) started at roughly `-54°`, could never reach its `+20°` target, and the optimizer flattened the surface to a zero tangent angle instead. The prediction now takes the angle from the *absolute* cosine, folding it into `[0, pi/2]` so it is invariant to the normal's orientation, and clips the `arccos` argument to guard its gradient singularity when the vectors align. `test_goal_angle.py` gains winding-invariance and upper-hemisphere regression tests that fail against the signed-cosine version.
- Fixed `mesh_to_network` in `examples/pringle/pringle_temporal_horizontal.py`, which copied the solved mesh's residuals back with `node_attributes(node, names="rxyz", ...)`. The `"rxyz"` string is iterated character by character, so COMPAS zipped its four names `r, x, y, z` against the three residual values and overwrote each node's `x` and `y` coordinates with `rx` and `ry` right after they had been written correctly. Every support (and node) collapsed onto the residual field, which left one half of the vault (the `y = 6` end) effectively unsupported and wrecked the sequential optimization. The names are now the explicit list `["rx", "ry", "rz"]`. With the fix, a one-course run reproduces the reference network analysis's support layout (both `y`-ends anchored) in equilibrium.
