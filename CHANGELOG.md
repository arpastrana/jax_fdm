...
- Changed goals and constraints to be stateless: `Goal.init`/`Constraint.init` are removed, and `__call__` now resolves the element index and reads structure-derived data at evaluation time. `structure` is threaded into the hooks — `prediction(eq_state, structure, payload)` and `constraint(eq_state, structure, payload)` — riding `vmap` as an `in_axes=None` operand (never batched, sparse-safe). Goals/constraints carrying an extra per-element array (the vector goals/constraints, curvature) override `operand(structure)` to return a tuple payload the hook unpacks; the reference-vector scatter tables and the `_index`/`index` property are gone. This removes the latent bug where `init` mutated the user's `vector` into a scatter table (which `evaluate` would have made live). Key→index resolution is now vectorized (`indices_from_keys`, `argsort`+`searchsorted`) with no Python loops; it raises `KeyError` on absent keys, including edge pairs whose endpoints exceed the largest canonical node (the fold base spans the query too), pinned by `tests/test_graph_indices.py`. Post-jit runtime and compile time are unchanged (bit-identical loss on a fine monkey-saddle). Breaking for anyone calling `.init()`, reading `.index`/`.index_polygon`, or overriding `prediction`/`constraint`/`index_from_key` (now `prediction(eq_state, structure, payload)`, `_indices_from_keys`). Supersedes the earlier "goal index one-dimensional", "aggregate index dispatch through `index_from_structure`", and `key_index`-hook notes in this section.
