...
- Changed every package `__init__.py` from the inherited `from .x import *` + `__all__ = [name for name in dir() ...]` idiom to explicit one-name-per-line re-exports and a hand-listed `__all__`. The `dir()` sweep re-exported everything the wildcard imports pulled in — submodule names (`node`, `edge`, `angle`, ...) and incidental imports (`jnp`, `vmap`, `Any`, `Sequence`, geometry helpers, COMPAS classes, the `TargetLike`/`AnyFDDatastructure` aliases, the typing mixins) — so the star-exported package namespaces carried far more than the public API (`jax_fdm.goals` alone listed 103 names, ~40 of them noise). The explicit form makes each package's public surface the single source of truth the static checkers can see, and covers the whole re-export chain including the nested subpackages (`goals/node`, `constraints/edge`, `optimization/optimizers`, ...) and the three `has_backend`-gated `visualization` subpackages, whose `__all__` stays backend-adaptive by extending from the resolved subpackage `__all__`s. `EdgeForceDensityParameter` also stops being importable from `jax_fdm.optimization` — it only ever appeared there because the optimizer module imports it for its default parameter — so import it from its home package, `jax_fdm.parameters`; the examples, tests and documentation guide are repointed accordingly. This is a breaking change for imports of these incidental names from package namespaces instead of their home modules.
- Changed the intra-package base-class imports that read from a package's own half-initialized namespace (`from jax_fdm.goals import ScalarGoal` inside a goal module, and the equivalents in `constraints`, `optimization`, `equilibrium`, `losses` and `parameters`) to import from the concrete defining submodule instead (`from jax_fdm.goals.goal import ScalarGoal`). The old wildcard `__init__`s masked the latent circular-import fragility by hand-ordering their star imports so base classes bound first; explicit imports are alphabetized by isort, which surfaced it. Importing from the leaf module removes the load-order dependence entirely. Internal only — no public import path changes.
