...
- Stopped the change-watch timers of the sidebar components (number edits, color pickers, toggles) when retiring a spent viewer. Each bound component polls its widget on a 100 ms `QTimer` that outlived the eviction, firing into the next viewer's event loop against deleted C++ widgets and flooding the terminal with `RuntimeError: libshiboken: Internal C++ object already deleted`.
