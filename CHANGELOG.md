...
- Moved the `dev`, `docs` and `typecheck` extras to PEP 735 `[dependency-groups]`, leaving `viz` as the only user-facing extra. Contributor tooling no longer ships in the published metadata. `uv sync` installs all three groups by default (`[tool.uv] default-groups`); on pip use `pip install -e . --group dev` (pip 25.1+). Breaking for contributors who used `pip install -e ".[dev]"`.
- Adopted `uv` for the development workflow and committed `uv.lock` and `.python-version` (3.12), so every clone and CI job resolves the same dependency set. Users keep installing from PyPI with `pip`; the package build is unchanged. Conda is no longer required for development.
