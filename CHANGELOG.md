...
- Raised the `requires-python` floor from `>=3.11` to `>=3.12` and dropped the 3.11 classifier. Breaking. JAX 0.11.0 (2026-07-16) dropped Python 3.11, so a 3.11 install was silently frozen at jax 0.10.2 while 3.12+ users moved on. The CI matrix now runs 3.12 and 3.13 on all three runner operating systems. Pyright `pythonVersion` follows to 3.12; ruff infers its target from `requires-python`, so its `target-version` key is gone. At 3.12 numpy's `ArrayLike` includes the `Buffer` protocol, which the untyped compas `edge_coordinates` return no longer satisfies, so the typing shims in `datastructures/types.py` now declare it.
...
- Raised the `requires-python` ceiling from `<3.13` to `<3.14` and added the 3.13 classifier. Every dependency, extras included, resolves to a prebuilt 3.13 wheel on Linux, macOS and Windows. COMPAS runs there, but its upstream classifiers and CI stop at 3.12. Hopefully, it will be upgraded soon.
