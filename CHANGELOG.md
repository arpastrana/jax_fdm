...
- Added a cable-net prestress walkthrough (`docs/examples/cablenet.md`, in the Examples nav) for `examples/cablenet/cablenet.py`, promoted out of the `more.md` script list. Frames the example around why `force / edge_length` misses its target once the solve moves the geometry. Introduces `LossPlotter` and `OptimizationRecorder` in a convergence section, reading the two named error terms off the chart and noting that the plotted curves are `alpha`-weighted. Quantities carry kN and m units throughout, so the numbers read as engineering values rather than abstractions.
- Added a square cable-net example (`examples/cablenet/cablenet.py`) that prestresses a corner-supported tensile net until its boundary cables reach a target force and its interior cables a target length. Sets the starting force densities as a target force over a rest length, then drives the two `MeanSquaredError` terms (`EdgeForceGoal` on the boundary, `EdgeLengthGoal` in the interior) to zero with `LBFGSB`, while flooring every `EdgeForceDensityParameter` at `qmin=0.1`. The floor is well above zero on purpose: at `1e-3` the twenty interior edges along the two straight centerlines converge to nearly zero force, since a straight cable with collinear neighbors is self-equilibrating at any force and its density is a near-null direction. Raising the floor removes the slack cables and reaches a lower final loss than the looser bound did. Records the optimization with an `OptimizationRecorder` callback and charts the loss breakdown with `LossPlotter` before opening the viewer.

### Changed

- Declared `edge_coordinates` on the compas typing shims in `datastructures/types.py`. Pyright needs it when checking at Python 3.12 or later, where numpy's `ArrayLike` includes the `Buffer` protocol that the untyped compas return no longer satisfies. Dropped the ruff `target-version` key, which ruff infers from `requires-python`.
- Moved the `dev`, `docs` and `typecheck` extras to PEP 735 `[dependency-groups]`, leaving `viz` as the only user-facing extra. Contributor tooling no longer ships in the published metadata. A bare `uv sync` installs only the runtime dependencies (`[tool.uv] default-groups = []`), the same as `pip install -e .`; contributors add `--all-groups --extra viz`. On pip use `pip install -e . --group dev` (pip 25.1+). Breaking for contributors who used `pip install -e ".[dev]"`.
- Adopted `uv` for the development workflow and committed `uv.lock`, so every clone and CI job resolves the same dependency set. Users keep installing from PyPI with `pip`; the package build is unchanged. Conda is no longer required for development.
- Moved CI onto `astral-sh/setup-uv`: every job runs `uv sync --locked` with only the group it needs (`dev`, `docs` or `typecheck`) plus the `viz` extra, and invokes tools through `uv run` under `UV_NO_SYNC=1` so a run step never re-syncs the default groups back in. `--locked` fails a job when `uv.lock` drifts from `pyproject.toml`. The release workflow builds with `uv build`. The `build` workflow also runs weekly on a schedule, resolving against the newest dependencies instead of the lock, as an early warning for upstream breakage.
- Pinned `ruff` in the `dev` group and switched the pre-commit hooks to run it through `uv run`, so `uv.lock` is the single source of the ruff version. The whitespace hooks now also skip `.svg` files, since the matplotlib-exported site logo ends every path-data line with a space and a full-tree hook run would rewrite 1313 of its lines. The unpinned group had resolved to a newer ruff whose formatter disagreed with the remote hook on seven files. Dropped `build` and `twine` from the group, since the release workflow builds with `uv build` and publishes through the PyPI action.
- Added the `uv add jax-fdm` one-liner next to `pip install` in the README installation section, with a pointer to the `uv sync` development setup on the installation page. The installation page distinguishes `uv add`, which records the dependency in a project, from `uv pip install`, which installs into an existing virtual environment.
- Split the installation page into an Install section for users (`pip`, `uv add`) and a Develop section for contributors (`uv sync`, `uv run`, with the pip `--group` fallback). CONTRIBUTING follows the same workflow.
- Raised the `requires-python` ceiling from `<3.13` to `<3.14` and added the 3.13 classifier. Every dependency, extras included, resolves to a prebuilt 3.13 wheel on Linux, macOS and Windows. COMPAS runs there, but its upstream classifiers and CI stop at 3.12. Hopefully, it will be upgraded soon. The CI matrix runs 3.11 and 3.13 on all three runner operating systems, plus 3.12 on Ubuntu. Python 3.11 stays supported although jax 0.11.0 (2026-07-16) dropped it, so a 3.11 install resolves to the last compatible jax, 0.10.2, and `uv.lock` carries both resolutions.
- Raised the `numpy` floor to `>=2.1`, `matplotlib` to `>=3.9.2` and `scipy` to `>=1.15`, the first releases with cp313 wheels and, for scipy, the floor jax 0.11.1 declares. The `DifferentialEvolution` and `DualAnnealing` optimizers now pass `rng` instead of the `seed` keyword scipy deprecated, which the 1.15 floor allows. Dropping the `seed` pragma exposed that `OptimizationProblem.tol` was typed `float | None` although every producer passes a float, so it is now `float`. Every other floor was already 3.13-ready as declared. Checked with `uv pip compile --resolution lowest-direct --only-binary :all:`.
- Changed the `build` and `docs` workflow concurrency so only pull request runs share a group. Every other event keys on `github.run_id`, so a run that never completes cannot block later pushes to main. The docs deploy keeps its serialization in a job-level `docs-deploy` group.
- Added a `workflow_dispatch` trigger to the `build` and `docs` workflows, so both can be run by hand. The docs `deploy` job now runs on any non-pull-request event, so a manual run on main redeploys the `dev` docs.
...
- Removed the `IPOPT` optimizer and the `ipopt` extra (`cyipopt`). Breaking: `from jax_fdm.optimization import IPOPT` no longer resolves. `cyipopt` ships no wheels and needs a system Ipopt library, no example or test used it, and `SLSQP` and `TrustRegionConstrained` cover constrained problems. `OptimizationProblem.bounds` and `parameters_bounds` narrow to scipy `Bounds`, and `OptimizationProblem.constraints` to a list of `NonlinearConstraint`, the only producers left.
...
- Added an `evaluate(datastructure)` family for prototyping without an optimization: `Goal.evaluate`/`Constraint.evaluate` return the goal state / constrained values read off a datastructure's current geometry, `Error.evaluate` sums its raw goals (no collection grouping), `Loss.evaluate` adds the regularizers on top, and `Parameter.evaluate` aliases `value`. Backed by a new `datastructure_state(...) -> DatastructureState` factory (exported from `jax_fdm.equilibrium`) that assembles the state/structure/parameters from the COMPAS layer without solving. Node loads read as-is; no edge/face tributary re-aggregation. `tests/test_evaluate.py` pins parity vs the manual pipeline, dense/sparse agreement, and that a vector goal/constraint is not mutated by evaluation.
...
- Added `VertexCurvatureConstraint`, a mesh-only constraint bounding the discrete angle-deficit curvature at a vertex over its ordered one-hop neighbor ring.
...
- Added a gridshell-planarization example (`docs/examples/gridshell_planarization.md`, `examples/gridshell/gridshell_planarization.py`) that flattens a compression shell's quad faces for cheaper cladding. Builds the loss goal by goal: `MeshPlanarityGoal`, then `MeshSmoothGoal` as a fairness regularizer, then `VertexPointGoal` shape matching, and finally `VertexSupportXParameter` support finding. Reports flatness with COMPAS `mesh.face_flatness` and colors faces by a shared planarity scale.
...
- Added contract-teaching errors at the extension seams: `Goal.__call__` raises `ValueError` on a goal/prediction shape or target-count mismatch, and `Constraint.__call__` raises `ValueError` when a per-element constraint is fed several keys (its value is not scalar).
- Added a `facecolor` keyword to the `FDMesh` scene objects across all three backends (viewer, notebook, plotter), so the mesh faces paint with one color or a per-face `{face: Color}` map instead of a single default shade. It was silently dropped before: the viewer and notebook built their inner mesh object with a hardcoded signature, and only the plotter (which inherits its face pass) honored it. Made an explicit typed parameter (`FaceColorSpec`, new in `visualization/style.py`) on each, which also keeps it out of `**kwargs` and off the wireframe base. Default path unchanged. Pinned by the facecolor tests in `tests/test_scene_objects.py` and `tests/test_plotter_scene_objects.py`.
- Added an `edgeopacity` keyword to the FD viewer scene objects (`FDMeshObject`/`FDNetworkObject`), so the wireframe edges can be made opaque over the faces. Sets the Edges category alone; points, loads and reactions keep the shared `default_opacity` (0.75). Viewer-only, since the notebook (pythreejs) and plotter backends have no opacity path. Default path unchanged. Pinned by the edgeopacity tests in `tests/test_scene_objects.py`.
...
- Changed default face opacity in `viewers/FDMeshObject` from `0.4` to `1.0`, and set global `COLOR_EDGE` to gray across viz backends.
- Changed goals to vmappable JAX pytrees (`Goal` is now an `equinox.Module`): `key`/`target`/`weight`/`vector` are dynamic array leaves stored unbatched (one element's value), so a custom goal reads `self.<attr>` and the element `index` directly in `prediction` with no `operand`/`payload` seam. `Goal.__call__` is the linear per-element body (index, prediction, target, shape check, state) and returns raw per-element shapes; the evaluation `vmap` lives in `Error.__call__`, so a collection is `tree_stack`ed (a `tree_map(jnp.stack)` over same-type goals) and the error term maps the per-element error over it and sums. Per-element vs aggregate is the class-level `is_aggregate` flag alone, never the key's Python type. Loss is bit-identical (scalar/vector/aggregate/weighted/edge/mesh all verified to full float64); full suite and the mesh/network examples pass. Breaking for third-party goals overriding `operand`, relying on target/weight reshaping to `(-1, …)`, reading a batched shape off a lone goal (now unbatched), passing `vector` positionally to `EdgeAngleGoal`, or passing `key` to a whole-structure goal (now a `TypeError`). Supersedes the `operand`/`payload` and `prediction(…, payload)` parts of the statelessness note below.
- Changed constraints to vmappable JAX pytrees the same way (`Constraint` is now an `equinox.Module`): `key`/`bound_low`/`bound_up`/`vector`/`polygon` are dynamic leaves stored unbatched, `index`/`keys_from_structure`/`constraint(eq_state, structure, index)` mirror the goal hooks, and key resolution runs under the evaluation `vmap` via the traceable `indices_from_keys` (the curvature polygon resolves its neighbor ring there too). `Constraint.__call__` takes `(eqstate, structure)` and returns the raw per-element value; the optimizer solves once then owns the `vmap` and ravel over a `tree_stack`ed collection, and `collect_constraints` gives each aggregate its own singleton collection. The `Collection` reflection class and `operand`/`indices`/`index_from_structure` are gone. A per-element constraint fed a list of keys now stores cleanly and raises a `ValueError` at evaluation (no construction guard), since a constraint lacks a goal's prediction shape check. Breaking for `Constraint.__call__(params, model, structure)` callers (now `(eqstate, structure)`), third-party constraints overriding `operand`/`index_from_structure`, and `Collection` importers. Pinned by the constraint blocks in `tests/test_evaluate.py` and `tests/test_goals_vertex.py`.
- Removed the `ScalarGoal`/`VectorGoal` mixins and the `_is_vector` flag: a goal now declares its shape implicitly through what `prediction` returns (`()`/`(target,)` per element), and the sole invariant (goal vs prediction shape) is enforced at evaluation in `Goal.__call__`. Breaking for goals subclassing `ScalarGoal`/`VectorGoal` (drop the mixin) and for the target-shape mismatch, now a `ValueError` at evaluation rather than construction.
- Simplified goal key→index resolution to one worker, `Goal.index()` (was `Goal.indices()`): it calls the subclass hook `keys_from_structure` (renamed from `index_from_structure`, itself earlier renamed from `index_from_model` since no implementation read the model) and resolves the key against those canonical keys with a traceable `argsort`+`searchsorted` resolver that runs inside the evaluation `vmap` (canonical prep stays NumPy at trace time; a missing key raises via `equinox.error_if` at evaluation rather than a trace-time `KeyError`). Both resolvers moved to a new `equilibrium/indexing.py`: the traceable one is now `indices_from_keys` (public), and the legacy pure-NumPy off-trace resolver it was a twin of is now the private `_indices_from_keys`, kept for the constraints. Resolving inside the map drops the aggregate leading-axis branch: the key's own shape distinguishes a per-element goal (scalar key) from an aggregate (whole row). A subclass supplies its vocabulary alone; the pure-forward `index_from_key`/`_indices_from_keys` helpers and the collapse-to-`int`/`tuple` round-trip are gone, and whole-structure goals override `index()` to return the `-1` sentinel. `tree_stack` moved to `optimization/collections.py`. Breaking for third-party goals overriding `index_from_structure`/`index_from_model` (now `keys_from_structure`, returning canonical keys) or calling `_indices_from_keys`/`indices`. Constraints keep `index_from_structure`/`_indices_from_keys`/`indices` until their migration.
- Changed goals and constraints to be stateless: `Goal.init`/`Constraint.init` are removed, and `__call__` resolves the element index and reads structure-derived data at evaluation time, threading `structure` into the hooks as an `in_axes=None` `vmap` operand (never batched, sparse-safe). This removes the latent bug where `init` mutated the user's `vector` into a scatter table. Key→index resolution is vectorized (`_indices_from_keys`, `argsort`+`searchsorted`, no Python loops) and raises `KeyError` on absent keys, including edge pairs whose endpoints exceed the largest canonical node, pinned by `tests/test_graph_indices.py`. Post-jit runtime and compile time unchanged (bit-identical loss on a fine monkey-saddle). Breaking for anyone calling `.init()` or reading `.index`/`.index_polygon`. (The goal hook signatures and `operand`/`payload` seam described here are superseded for goals by the pytree note above; constraints still use them.)
...
- Changed every dependency from a bare name to a version floor, so a resolver cannot pick a release the library does not run on. `lineax>=0.1.0` is the binding constraint on the core set: it requires jax 0.6.1, which requires numpy 2.0. The floors were validated by resolving the lowest-direct set and running the full suite against it, which caught three that were too low to import: `jaxtyping` below 0.2.33 breaks `import equinox` on a metaclass layout conflict, `compas_viewer` below 2.0 has no `components.sidebar` module (and its 1.x line still reads the long-gone `compas.utilities.flatten`), and `pythreejs` below 2.1 imports `collections.Sequence`, removed in Python 3.10. `pythreejs` carries a floor here because `compas_notebook` leaves it unpinned.
- Changed the license metadata to the PEP 639 `license = "MIT"` and `license-files` fields, replacing the free-text table and the deprecated `License ::` classifier. Raises the build requirement to `setuptools>=77.0.3`.
...
- Changed scalar goals to return a bare `()` prediction and target (the coordinate, force, length, load-path and residual-force goals migrated off their old `(1,)` slices), so a custom `goal()` sees the prediction's true per-element shape and the `Goal.__call__` shape check compares raw hook shapes. A scalar goal handed a one-element list target (`[0.5]`) now raises where a bare `0.5` passes. Weight is stored as a bare scalar, so under the per-element `vmap` in `Error.__call__` the error kernels see one element's `()` weight against its raw prediction; this fixes a latent crash where a weighted rank-2 (frame) goal collection could not broadcast a `(N, 1)` weight against an `(N, 2, 3)` prediction. Initializing a `Node*` goal or constraint against a mesh (or a `Vertex*` one against a network) raises a `TypeError` pointing to the right counterpart. Breaking: mesh call sites must switch to the vertex classes; third-party scalar goals returning `(1,)` should return `()`.
- Changed aggregate goals and constraints to declare a class-level `is_aggregate = True` flag (replacing the hand-written boilerplate and the negated `is_collectible` instance attribute); `collect_goals`/`collect_constraints` give each aggregate its own singleton collection. `NetworkGoal`, `MeshGoal` and `NetworkConstraint` declare the flag too. `EdgesForceEqualGoal`, `EdgesLengthEqualGoal`, `NodesColinearGoal` and `NodesCurvatureGoal` shrink accordingly. Breaking for third-party aggregates (declare `is_aggregate = True` at class level) and for code reading `is_collectible`.
