...
- Ported the notebook backend to compas_notebook 2.x (`>= 0.11`). `NotebookViewer` now subclasses `compas_notebook.viewer.Viewer` (the compas 1.x `compas_notebook.app.App` is gone) and mirrors the 3D viewer wrapper: `add` dispatches an `FDNetwork`/`FDMesh` by type through the new notebook artists and returns the artist, everything else goes to `scene.add`, and the `width`/`height`/`show_grid`/`viewport`/`camera_position`/`camera_target` convenience kwargs fold into a `compas_notebook.config.Config` (camera setup replaces the removed `set_camera`; compas_notebook's `Config.view` is a shared class attribute, so the wrapper assigns a fresh `ViewConfig` per viewer). Added `FDMeshNotebookArtist` so meshes render in notebooks with full force-density metadata too, on a shared `FDDatastructureNotebookArtist` base. Because every notebook scene object costs Jupyter-widget overhead, the notebook artists batch each element category (edges, nodes/vertices, loads, reactions) into a single joined mesh with per-face colors keyed by element — a whole network renders as a handful of pythreejs objects instead of two per element. Notebook rendering is draw-once (no scene tree, no animation loop).
- Registered the force-density datastructures with the COMPAS 2.x scene system, so a *native* COMPAS scene — a bare `compas_viewer.Viewer` or a compas_notebook scene, not just the jax_fdm wrappers — renders an `FDNetwork`/`FDMesh` through the jax_fdm artists (`scene.add(fdmesh, edgecolor="force")` just works). The scene objects are thin interop adapters over the artists: in the Notebook context they draw the batched buffers flat, and in the Viewer context they are a scene-group veneer under which the viewer artist parents its usual per-category subgroups, preserving the foldable tree. Registration runs automatically at `import jax_fdm.visualization`, gated per installed backend, and forces COMPAS's built-in scene-object discovery first — COMPAS only auto-discovers when its registry is empty, so registering into a fresh registry would mask every built-in type; jax_fdm also cannot register through the plugin system itself, because discovery only scans `compas*`-named packages.
...
- Aliased `edgewidth` to `linewidth` for plain-geometry adds in the 3D `Viewer`, so `edgewidth` is the one edge-width vocabulary across FD and plain adds alike: the FD artists consume `edgewidth` (world-space cylinder width; scalar, per-edge dict, or a `(min, max)` tuple remapped by force), while compas_viewer's native scene objects only read `linewidth` (screen-space pixels) and silently store-but-ignore the `edgewidth` they inherit from `compas.scene`. An explicit `linewidth=` still wins. Note that GL drivers (notably macOS) may clamp line widths to 1px regardless.

### Changed

- Extracted a shared `FDDatastructureArtist` base (and, for the viewer backend, `FDDatastructureViewerArtist`) from `FDNetworkArtist`, mirroring the datastructure layer where `FDNetwork` and `FDMesh` already share `FDDatastructure`. The base owns all the force-density rendering logic (point/edge colors, widths, load and reaction vectors, scene grouping and in-place animation updates) and reaches the point-like entities (nodes/vertices) through `_point_*` hooks, so the only thing the subclasses supply is the node-vs-vertex vocabulary. `FDNetworkArtist`/`FDNetworkViewerArtist` keep their names and their `network`/`nodes` attributes as aliases, so existing code and the deferred notebook/plotter backends are unaffected. `viewer.add` now dispatches purely by type, with no `as_wireframe`/`as_mesh` flags: to draw a force-density datastructure as plain geometry, convert it to its COMPAS base first, e.g. `viewer.add(mesh.copy(cls=Mesh))` or `viewer.add(network.copy(cls=Network))` (the examples were updated accordingly). Since COMPAS 2.x aliases `Network` to `Graph`, such a converted network would otherwise show up as "Graph" in the viewer's scene tree, so `viewer.add` defaults its display name to "Network" (any explicit `name=` still wins).
...
- Raised the `requires-python` ceiling from `<3.12` to `<3.13` and added the 3.12 classifier, now that COMPAS 2.x (which lists 3.12) and JAX (no upper cap) both support it. Added a Python 3.12 runner to the CI build matrix accordingly; 3.12 was previously blocked by the `compas<2.0` pin importing the removed `distutils` module.
- Decoupled the base visualization artist from COMPAS. `FDNetworkArtist` used to subclass `compas.artists.NetworkArtist`, which COMPAS 2.x removed (it moved to `compas.scene`), so importing it unconditionally broke `import jax_fdm.visualization` — and therefore the whole test suite via `conftest.py`. It is now a plain, COMPAS-free base class that owns the force-density-specific computation (node/edge colors, edge widths, load and reaction vectors) and exposes it as backend-neutral dictionaries; concrete backends subclass it and implement the `draw_*` methods. It therefore always imports regardless of which backend, if any, is installed. `remap_values` in that artist was also moved to `compas.itertools`.
- Migrated the example scripts to the COMPAS 2.x API alongside the source and tests: `Polyline.divide_polyline()` became `Polyline.divide()` in `examples/arch/arch.py`.
- Repointed the `viz` optional-dependency extra from the discontinued `compas_view2==0.7.0` (COMPAS 1.x only, which made `pip install jax_fdm[viz]` unresolvable against the new `compas>=2.15` pin) to its COMPAS 2.x successor `compas_viewer>=1.0`, making the extra installable again. With the notebook port, the extra also grew `compas_notebook>=0.11`.
- Ported the 3D `Viewer` off the discontinued `compas_view2.app.App` onto `compas_viewer`. The wrapper now subclasses `compas_viewer.Viewer` (camera control, the `on` frame loop, `show` and recording all work natively) and only overrides `add` to route an `FDNetwork` through the new `FDNetworkViewerArtist`; the `width`/`height`/`show_grid` convenience kwargs are folded into a `compas_viewer.config.Config`. The viewer artist builds plain COMPAS geometry (spheres, cylinders, arrow meshes) and keeps handles for in-place animation updates. The custom `Arrow` shape moved up to `jax_fdm/visualization/shapes.py` so every backend can share it, and the backend/`conftest`/example wiring was repointed from `compas_view2` to `compas_viewer` (`viewer.view.camera` became `viewer.renderer.camera`, and `rotation[2]` became `rotation.z`). The notebook backend got its own port in this release (see Added).
- Set the viewer's default render mode to `"lighted"` and bumped the `Arrow` mesh resolution default from `u=4` to `u=8` for smoother load and reaction vectors. Also fixed the arrow head detaching from its body under COMPAS 2.x: `Cone.from_line_and_radius` centers the base circle on the axis line's midpoint (apex a full length beyond), so the head's axis line is now centered on the head base to seat the base there and land the tip on the arrow end point.
- Grouped the viewer's `FDNetwork` elements into a single foldable scene-tree entity. `FDNetworkViewerArtist` previously added every edge, node, load and reaction as a loose scene object, cluttering the viewer tree with hundreds of bare `Cylinder`/`Sphere`/`Mesh` rows. They are now parented into per-category subgroups ("Nodes", "Edges", "Reactions", "Loads") under one group named after the network, using `compas_viewer`'s native scene groups. The network reads as one collapsed node in the tree; toggling the parent's visibility hides the whole network (the shader cascades `show` down the parent chain), each category can be toggled independently, and individual elements remain as leaves named by their key (`Edge (u, v)`, `Node (u)`, `Load (u)`, `Reaction (u)`).
- Updated the README and the bug-report issue template for the migration: dropped the COMPAS 1.x / `compas_view2` conda install step (COMPAS 2.x now installs transitively via `pip`), listed Python 3.12 as supported, and noted Matplotlib as a core dependency rather than part of the `viz` extra.

### Fixed

- Fixed `viewer.add(fdnetwork, name="...")` ignoring the `name` keyword for an `FDNetwork` or `FDMesh`: the force-density viewer artists always named their parent scene group after the datastructure (`FDNetwork`/`FDMesh`), silently swallowing any explicit `name=`. The artists now accept `name` and it takes precedence over the datastructure's own name in the viewer's scene tree.

### Removed

- Removed the `examples/arch/arch_plotter.py` example. It rendered its arch through `compas_plotters`, which is discontinued and has no COMPAS 2.x release; the 3D `Viewer`-based `examples/arch/arch.py` covers the same case.
- Removed `jax_fdm/visualization/notebooks/register.py`: the compas 1.x `Artist.register` factory it targeted no longer exists; the compas 2.x scene registration lives in `jax_fdm/visualization/scene.py` and the per-backend `scene.py` modules. The mesh viewer artist's "Faces" surface (and its notebook counterpart) now pins the native mesh scene object via `sceneobject_type`, since a plain `scene.add(fdmesh)` would dispatch back to the newly registered force-density adapter and recurse.
