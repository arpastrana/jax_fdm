...
- `EdgeAngleGoal` clips the cosine fed to `arccos`, whose value and gradient are singular when the edge is parallel to its reference vector: in floating point the cosine of two parallel vectors can overshoot 1 by a few ulps, and the prediction itself came out `nan`. A new regression test evaluates the goal on edges exactly parallel to the reference vector.
