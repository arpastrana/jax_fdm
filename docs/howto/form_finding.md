...
A node or vertex load is a plain point load: whatever we set is what the node carries.
Edge and face loads are **line** and **area** loads, magnitudes per unit length or area, that the model resolves onto nodes for us.
...
`fdm` takes in several keyword arguments that shape *how* equilibrium is computed.
The defaults suit most problems, so reach for these only when we need them:
...
But the actual solve happens in JAX, on plain, immutable arrays.
...
Generally, we do not build these by hand for a basic workflow.
However, they are exactly what our custom goals and constraints read from, so they deserve an in-depth explanation.
...
The structure is the bridge: `fdm` converts our datastructure into one before solving.
...
This is the object behind the key-versus-index duality we will meet in the [goals](goals.md#keys-versus-indices) guide.
...
`fdm` picks the right one for us from the datastructure we pass and the `sparse` flag.
...
For a plain `fdm` call these come straight off our datastructure. Under `constrained_fdm`, they are exactly what the optimizer is free to change, one number at a time.
...
The one time it steps aside is when we add constraints: constrained form-finding runs on the dense model, and passing constraints with `sparse=True` quietly switches us to dense (with a printed heads-up).
...
- To register a target quantity in JAX FDM it does not measure yet, write a [custom goal](custom_goals.md) or [custom constraint](custom_constraints.md).
