...
- Added `tests/test_goal_angle.py`, guarding the shape contract of angle goals: it drives constrained form finding with multiple `EdgeAngleGoal`s (the multi-edge path that regressed) and pins that the goal's prediction shape matches its target. The test fails against the pre-fix scalar `angle_vectors`.
...
- Fixed a shape regression in the angle goals (`EdgeAngleGoal`, `NodeNormalAngleGoal`, and thus `NodeTangentAngleGoal`). When `angle_vectors` was refactored to delegate to `cosine_vectors`, its output lost the trailing axis and became a scalar, so a goal's prediction came out `(N,)` while the `ScalarGoal` target stayed `(N, 1)`, raising an assertion in `Goal.__call__`. The goal predictions now wrap `angle_vectors` in `jnp.atleast_1d`, restoring the `(N, 1)` shape; `angle_vectors` stays scalar so `angles_polygon`/`FaceRectangleGoal` are unaffected.
