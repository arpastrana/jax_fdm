...
- Committed the sources of the three Colab notebooks linked in the README to a new `notebooks/` folder, migrated to the COMPAS 2.x API (the published versions still targeted compas 1.x: `Polyline.divide_polyline`, `compas.utilities.pairwise`, the 2D `Plotter`, `as_wireframe=True`). The arch notebook renders through `NotebookViewer(viewport="top")` instead of the discontinued plotter. The vault notebook's `butt*.json` inputs were lost from `main` (they now 404); its network files turned out to be identical to the committed `vault.json`/`vault0.json`, so it downloads those instead, and the target surface was recovered from git history as `data/json/vault_mesh.json`. `pip install` in the notebooks also pulls `compas_notebook` explicitly, since Colab does not install jax_fdm's `viz` extra.
