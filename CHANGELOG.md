...
- Created `EvolutionaryOptimizer()` class for other evolutionary algorithms to subclass it. This new parent class implements a specific `solve()` method that accounts for the different naming convention of the objective function in `scipy.minimize` (`fun` vs. `func`). Moreover, the new method does not try to calculate the gradient norm after completion since the gradient is unnecessary for evolutionary, gradient-free optimization.
