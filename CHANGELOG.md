...
- Ported the 3D `Viewer` off the discontinued `compas_view2.app.App` onto `compas_viewer`. The wrapper subclasses `compas_viewer.Viewer` and only overrides `add` to route the force density datastructures through their viewer artists; the `width`/`height`/`show_grid` kwargs fold into a `compas_viewer.config.Config`. The custom `Arrow` shape moved to `jax_fdm/visualization/shapes.py` so every backend can share it, and the backend, `conftest` and most example wiring was repointed (`viewer.view.camera` became `viewer.renderer.camera`, `rotation[2]` became `rotation.z`); the animation examples still await their port.
