...
- Repointed the `viz` optional-dependency extra from the discontinued `compas_view2==0.7.0` (COMPAS 1.x only, which made `pip install jax_fdm[viz]` unresolvable against the new `compas>=2.15` pin) to its COMPAS 2.x successor `compas_viewer>=1.0`, making the extra installable again.
- Ported the 3D `Viewer` off the discontinued `compas_view2.app.App` onto `compas_viewer`. The wrapper now subclasses `compas_viewer.Viewer` (camera control, the `on` frame loop, `show` and recording all work natively) and only overrides `add` to route an `FDNetwork` through the new `FDNetworkViewerArtist`; the `width`/`height`/`show_grid` convenience kwargs are folded into a `compas_viewer.config.Config`. The viewer artist builds plain COMPAS geometry (spheres, cylinders, arrow meshes) and keeps handles for in-place animation updates. The custom `Arrow` shape moved up to `jax_fdm/visualization/shapes.py` so every backend can share it, and the backend/`conftest`/example wiring was repointed from `compas_view2` to `compas_viewer` (`viewer.view.camera` became `viewer.renderer.camera`, and `rotation[2]` became `rotation.z`). The notebook backend still targets the compas 1.x API and now degrades to the `null_viewer` on a compas_notebook 2.x install pending its own port.
