...
class ConstrainedOptimizer(Optimizer):
    ...
    def constraints(self, constraints, model, structure, params_opt):
        ...
        for constraint in constraints:
            ...
            fun = partial(self.constraint, constraint=constraint, structure=structure)
            jac = jit(jacfwd(fun))
    ...
    def constraint(self, params_opt, constraint, structure):
        """
        A wrapper around a constraint callable object.
        """
        model = self.parameters_fdm(params_opt)

        return constraint(model, structure)
