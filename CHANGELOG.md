...
- Added a cablenet-on-an-arch example (`docs/examples/cablenet_arch.md`, `examples/cablenet_arch/cablenet_arch.py`) inspired by `compas_fd`. Builds a NURBS-free circular arch with numpy, then stacks three goals into one loss: `VertexResidualPlaneGoal` on a rotated arch, `EdgeForceGoal` to equalize the boundary cables, and `VertexZCoordinateGoal` for walk-under clearance.
