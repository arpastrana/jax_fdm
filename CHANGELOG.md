...
- Added a shape-matching example walkthrough (`docs/examples/shape_matching.md`) that approximates a doubly-curved vault with a compression-only network, contrasting an initial constant-force-density guess against the optimized `NodePointGoal` match.
