...
- Added `FDMeshArtist` and its `compas_viewer` backend `FDMeshViewerArtist`, so an `FDMesh` renders directly with its force density metadata (edge colors and widths, support colors, load and reaction vectors) plus a shaded surface for its faces.
- Converted the notebook backend to compas_notebook 2.x (`>=0.11`). `NotebookViewer` now subclasses `compas_notebook.viewer.Viewer`, dispatches an `FDNetwork`/`FDMesh` by type through the new notebook artists, and folds the `width`/`height`/`show_grid`/`viewport`/`camera_position`/`camera_target` kwargs into a `compas_notebook.config.Config`. The notebook artists batch each element category into a single pythreejs buffer with per-element colors to keep the Jupyter widget count low; rendering is draw-once (no scene tree, no animation loop).
- Added `FDMeshNotebookArtist` as a subclass of `FDDatastructureNotebookArtist`. 
- Registered `FDNetwork` and `FDMesh` with the COMPAS 2.x scene system, so a native COMPAS scene — a bare `compas_viewer.Viewer` or a compas_notebook scene — renders them through the jax_fdm artists.
...
- Extracted a shared `FDDatastructureArtist` base (and `FDDatastructureViewerArtist` for the viewer backend) from `FDNetworkArtist`, which keeps its name and its `network`/`nodes` aliases.
...
- Ported the 3D `Viewer` onto `compas_viewer`. The wrapper subclasses `compas_viewer.Viewer` and only overrides `add` to route the force density datastructures through their viewer artists; the `width`/`height`/`show_grid` kwargs fold into a `compas_viewer.config.Config`. 
...
- Fixed the freeze when visualizing several results in a row in one process (e.g. one per step of a sequential optimization). `compas_viewer.Viewer` is a process-wide singleton whose `running` flag survives closing the window, so every `scene.add` after the first `show()` triggered a full buffer and scene-tree rebuild over the accumulated objects (quadratic slowdown, or a crash against the dead GL context). The supported pattern is now to reuse one `Viewer` across shows: the new `viewer.clear()` empties the scene (including the artists and the picking-color registrations the parent never prunes) between shows, and `show()` rebuilds the GL render buffers on a re-show and resets `running` on return, so between-show adds stay lightweight. Sequential scripts construct the viewer once and run `clear()`/`add()`/`show()` per step; the camera carries over between steps.
