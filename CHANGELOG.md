...
- Ported the 2D plotter backend to the revived, standalone `compas_plotters` (COMPAS 2.x, scene-object based). `FDNetworkPlotterObject` and `FDMeshPlotterObject` draw a force density datastructure on the plotter's matplotlib canvas with the shared FD display semantics of `visualization/style.py`: edge color modes (`"force"`, `"fd"`, single color, per-edge dict), per-edge force-remapped widths (the plotter default bounds are `(0.5, 5.0)` points, since matplotlib linewidths live in points rather than world units), support coloring, and load and reaction arrows. The arrows are data-space polygons proportional to the arrow length — magnitude reads visually and arrows scale with the plot, unlike the constant on-screen glyph arrows of the upstream `VectorObject` — batched into one matplotlib collection per category, with below-tolerance arrows skipped. The objects report their arrow extents to `plotter.zoom_extents()`, and the datastructure vocabularies carry over from the other backends (`nodecolor`/`nodesize`/`show_nodes` on a network, `vertexcolor`/`vertexsize`/`show_vertices` plus `show_faces` on a mesh). Registered to the `"Plotter"` scene context; plain COMPAS `Graph`/`Mesh` objects keep dispatching to the upstream compas_plotters scene objects. `compas_plotters` is not on PyPI yet, so it is not part of the `viz` extra: install it from GitHub to enable 2D plotting, and without it `Plotter` degrades to the same warn-and-absorb null object as the other optional backends.
