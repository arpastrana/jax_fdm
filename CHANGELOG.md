...
- Added `tests/test_reference_solver.py`, which validates the solver against equilibria computed by an independent structural-analysis solver. Two self-stressed, well-conditioned reference networks are read from committed CSV fixtures; the tests assert that the imported state satisfies the equilibrium equations and that solving it with jax_fdm recovers the reference geometry to within `1e-8`. The reader is COMPAS-free, so the cross-validation survives the migration.
- Added `tests/test_optimizer_parameters.py`, covering optimization parameters that the suite did not exercise before: nodal load parameters (flattening an arch onto its support line with a colinearity goal, to `1e-9`), and grouped force-density, support, and load parameters. The group tests assert the defining invariant that a group collapses many degrees of freedom to a single shared value, sweeping support and load groups across all three Cartesian axes; the load sweep also guards the just-fixed Y-component regression by checking the untouched components stay put. All assertions are golden-free and use the LBFGSB optimizer.
...
- Added `tests/test_arch_benchmark.py`, validating the constrained optimizer against the analytical benchmark in Pastrana et al. (2026), *Differentiable force density method for the design of lightweight structures* (Computer Methods in Applied Mechanics and Engineering 458, doi:10.1016/j.cma.2026.118783), Appendix A.1. The inverse problem finds the rise of a compression-only planar arch that minimizes its load path, which has a closed-form solution, so no fixture is needed: the arch is built from scratch (no `compas.geometry`) with evenly-spaced nodes and a lumped uniform load. The optimized rise and load path are asserted against the analytical values (paper Eqs. A.2 and A.3) at `n_v=100` and `n_v=500`, reproducing the paper's reported accuracy (rise within 0.1%, load path within 1% by `n_v=100`), and a third test confirms the load-path error decreases monotonically as the arch is refined. The benchmark originates with Liew (2020) (doi:10.1016/j.istruc.2020.09.078); the DFDM paper reproduces it as a reference, as noted in the test docstring. Uses the LBFGSB optimizer and is golden-free.
- Added `tests/test_taylor_convergence.py`, validating the correctness of the analytical adjoints (ADA) from Pastrana et al. (2026), Appendix A.4, with the Taylor remainder convergence test. For a correct gradient the second-order remainder decays as `O(mu^2)`, i.e. a log-log slope of 2 as the step `mu` is halved. The analytical adjoints are a property of the equilibrium model, not of any structure, so the test exercises the same code paths on compact, generated geometry (a planar arch for the linear-solver adjoint, `tmax=1`; a meshgrid under follower loads for the URS adjoint, `tmax>1` with `is_load_local=True`) instead of the paper's domes, keeping it COMPAS-free and fixture-free. Scalar loss and gradient over the flat parameter vector are obtained from the optimizer's own problem hook (no full optimization). Each adjoint is checked at two discretization tiers, low and high (the high tier reaches 1,200 free degrees of freedom and is marked `slow`), asserting both the slope-2 convergence and that the analytical-adjoint gradient matches an unrolled automatic-differentiation gradient (`implicit_diff` True vs False). A negative-control test confirms a deliberately corrupted gradient fails the convergence check.
- Added `tests/test_liew_dome.py`, validating the constrained optimizer against the circular gridshell from Liew (2020), *Constrained Force Density Method optimisation for form-finding* (Structures 28, doi:10.1016/j.istruc.2020.09.078), Section 4.2. The pre-optimization dome (613 vertices, 1800 edges) is committed as a frozen fixture `tests/data/liew_dome.json` so the suite stays COMPAS-free and the node/edge ordering stays fixed (the live rebuild is order-unstable and SLSQP is order-sensitive). The base case matches the paper's published statistics exactly, and the volume-only optimization (`alpha_lp=1`, `alpha_lengths=0`) reproduces the paper's volume and length statistics while hard-asserting SLSQP convergence. scipy rewrote SLSQP from Fortran to C in 1.16.0, which changed its convergence behavior on this problem: the optimization converges (exit mode 0) under scipy < 1.16.0 but returns exit mode 4 from 1.16.0 onward, so the optimization test is gated with `skipif` to run only on scipy < 1.16.0 (the base case is unaffected and runs everywhere). The length-only and combined configurations are not version-robust and are documented but not asserted. The optimization test is marked `slow` so it can be skipped with `pytest -m "not slow"`.
...
- `Optimizer` now retains the backend result of its last solve on `self.result` (the scipy `OptimizeResult` for gradient-based optimizers), exposing the convergence status, message, and multipliers that `solve` previously discarded. `constrained_fdm` still returns the equilibrium network unchanged.
- Added Windows to the `build.yml` test matrix and rebalanced the Python versions. The matrix now runs Python 3.10 across all three operating systems (Ubuntu, macOS, Windows) plus a single Ubuntu job on Python 3.12, covering the supported floor and ceiling without growing the job count beyond the previous four. This closes a gap where Python 3.12 was advertised in the package classifiers but never exercised in CI.
