...
- Added an `evaluate(datastructure)` family for prototyping without an optimization: `Goal.evaluate`/`Constraint.evaluate` return the goal state / constrained values read off a datastructure's current geometry, `Error.evaluate`/`Loss.evaluate` sum their raw goals (no collection grouping) plus regularizers, and `Parameter.evaluate` aliases `value`. Backed by a new `equilibrium_state_from_datastructure(...) -> DatastructureEquilibrium` factory (exported from `jax_fdm.equilibrium`) that assembles the state/structure/model/params from the COMPAS layer without solving. Node loads read as-is; no edge/face tributary re-aggregation. `tests/test_evaluate.py` pins parity vs the manual pipeline, dense/sparse agreement, and that a vector goal/constraint is not mutated by evaluation.
