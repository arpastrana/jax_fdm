...
- Added a documentation guide on writing custom goals and constraints (`docs/custom_goals.md`), with recipes for scalar, vector and aggregate goals plus custom constraints, and the shape and init contracts the optimizer expects.
- Added `NodeResidualPlaneGoal` and `VertexResidualPlaneGoal`, which drive the residual force vector at a support toward a target plane described by its normal vector. The plane passes through the origin since the residual is a vector, and the goal returns the predicted residual direction's projection onto the plane, so the error term penalizes exactly the out-of-plane component. Both the prediction and the normal are unit-normalized: the error measures the sine of the angle between the residual and the plane, and cannot be reduced by shrinking the reaction magnitude instead of rotating the reaction into the plane. Tests in `tests/test_goal_residual_plane.py` pin the projection math, the magnitude and normal-scale invariances, the vertex vocabulary resolution, and an end-to-end arch optimization that rotates the support reactions into tilted target planes.
- Added contract-teaching error messages to the extension seams: `Collection` now explains the init-parameter-as-attribute convention when an attribute is missing (instead of a bare `AttributeError` from optimizer warmup), `Goal.__call__` raises a `ValueError` naming the `(1,)` scalar-prediction shape rule and the `jnp.atleast_1d` fix (instead of a bare assert), and the `VectorGoal` target setter raises a `TypeError` pointing to the scalar goal variant when given a single number (instead of an opaque reshape error). Widening the setter to handle the scalar branch also restores the `target` property's base-class contract, clearing the pyright override complaint on every vector goal.
...
- Changed the aggregate goals `NodesColinearGoal` and `NodesCurvatureGoal` to resolve indices via `self.index_from_structure` instead of `super().index_from_structure`, so the resolution respects the MRO of retargeted subclasses like `VerticesColinearGoal`.
