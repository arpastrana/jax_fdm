...
- Adopted two docs-build hardening measures from an audit of [exponax](https://github.com/Ceyron/exponax)'s documentation setup: `mkdocstrings-python` and `griffelib` are now pinned exactly in the `docs` extra alongside the packages that pull them in (griffe is the layer whose version-to-version behavior differences are the most likely to silently change what the API reference renders), and `pymdownx.superfences` gained the mermaid custom fence so docs pages can carry diagrams. Exponax's math-first API page pattern — the governing equation in LaTeX above each `:::` directive — is documented in the migration plan for the upcoming docstring phases.
