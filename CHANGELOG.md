...
- Added documentation how-to guides on writing custom goals (`docs/howto/goals.md`, with recipes for scalar, vector, moving-target and aggregate goals) and custom constraints (`docs/howto/constraints.md`), grouped under a new How-to Guides section in the docs navigation. The examples page split into an Examples section (`docs/examples/arch.md` walkthrough plus `docs/examples/more.md` galleries), with a redirect keeping the old `examples/` URL alive.
...
- Added typing-only mixin classes in `datastructures/types.py` (`FDMeshType`, `FDNetworkType`, `FDDatastructureType`), inherited as the first base of `FDMesh`, `FDNetwork` and `FDDatastructure`. The mixins leave no trace at runtime. For static checkers they re-declare what COMPAS's python-2 type comments misreport: the inherited constructors (`from_meshgrid`, `from_obj`, `from_json`, `from_vertices_and_faces`, `from_polyhedron`, `from_lines`, `from_nodes_and_edges`) and chainables (`copy`, `transformed`, `subdivided`) return the FD subclass, not base `Mesh`/`Data`, and the getter/setter dual accessors user scripts call (`vertices`/`nodes`, `vertices_where`/`nodes_where`, `vertex_attribute`/`node_attribute` and friends, `edges`, `edge_attribute`, the mesh-only `faces`/`face_attribute`, coordinate getters) have mode-split `@overload` signatures instead of one broad inferred union. Downstream, `FDMesh.from_meshgrid(...)` now types as `FDMesh`, so FD-specific method calls and `fdm(mesh)` type-check; in-package, the mixin declarations retire all ten `FDDatastructure` attribute-access warnings and about thirty `# pyright: ignore` suppressions across the datastructures, parameters, structures, and visualization modules. `OptimizationRecorder.from_json` gets the same `Self`-returning treatment. Non-overload stub bodies raise `NotImplementedError` rather than `...`, since pylint's inference reads a `...`-bodied stub as returning `None` and flags call sites with E1111/E1133; the stubs never execute either way. The two runtime `edges()` passthrough overrides on `FDMesh` and `FDNetwork`, which existed only to re-type `super().edges(data)`, are deleted; the FD-owned getter/setter duals (`vertices_supports`/`vertices_fixed`, `nodes_supports`/`nodes_fixed`/`nodes_anchors`) gained real overload pairs so their getter mode types as a plain iterator instead of `Iterator | None`.

### Changed

- Changed the pyright scope to also check `examples/` — the downstream-user perspective the typing mixins exist for — and restored `reportAttributeAccessIssue` to its default error severity, since the dynamic-attribute warnings it silenced are gone. The examples were brought to zero diagnostics with behavior-neutral edits: `recorder is not None` guards instead of re-testing the `record` flag, plain point-pair targets instead of COMPAS `Line` objects, and in `dome.py` a rename of the shadowing `optimizer = optimizer()` rebind plus a guard reorder so the checker can prove `recorder` is bound where it is read (the old guard order was runtime-safe but not provably so).
- Changed `fdm` and `constrained_fdm` to bind their return type to their input type through the `AnyFDDatastructure` type variable (bound to `FDNetwork | FDMesh`): a network in types a network out, a mesh a mesh — which is what the runtime returns. The variable threads through the internal helpers so the binding propagates without casts, and call sites no longer need `isinstance` narrowing to use network- or mesh-specific methods on the result.
- Cleaned up the star-exported package namespaces while keeping the compact `from .x import *` package `__init__.py` idiom. Two coordinated changes do it. First, every leaf module now declares its own `__all__` next to the classes it defines, so `from .leaf import *` pulls only that module's public API and never its incidental imports (`jnp`, `vmap`, `Any`, `Sequence`, geometry helpers, COMPAS classes, the `TargetLike`/`AnyFDDatastructure` aliases, the typing mixins); 64 leaf modules that lacked one gained it, matching the 37 that already had it, so the convention is now uniform. Second, each package `__init__.py` swaps the `__all__ = [name for name in dir() ...]` sweep for `__all__ = [name for name, value in vars().items() if not name.startswith("_") and not isinstance(value, _ModuleType)]`, which additionally drops the submodule names (`node`, `edge`, `angle`, ...) that a star-import binds into the package namespace — the one kind of noise a leaf `__all__` cannot suppress. Together these shrink each package's public surface to its real API (`jax_fdm.goals` from 103 exported names to 61) without hand-listing every symbol. `EdgeForceDensityParameter` stays importable only from its home package `jax_fdm.parameters`, not `jax_fdm.optimization` where it never had a leaf `__all__` entry; the examples, tests and documentation guide import it from there. The static `reportUnsupportedDunderAll` pyright warning on each package `__init__` is accepted: pyright cannot evaluate any computed `__all__`, and the compact form is the deliberate tradeoff.
- Changed the intra-package base-class imports that read from a package's own half-initialized namespace (`from jax_fdm.goals import ScalarGoal` inside a goal module, and the equivalents in `constraints`, `optimization`, `equilibrium`, `losses` and `parameters`) to import from the concrete defining submodule instead (`from jax_fdm.goals.goal import ScalarGoal`). The compact wildcard `__init__`s used to lean on a hand-ordered star-import sequence so base classes bound before their subclasses; importing each base from its leaf module removes that load-order dependence, so the package `__init__` star imports can be freely alphabetized by isort without breaking. Internal only — no public import path changes.
- Renamed the module-level `solvers` dict in `equilibrium/sparse.py` to `_solvers`. As a public name it was pulled in by `from .sparse import *` and shadowed the sibling `solvers` subpackage in the `jax_fdm.equilibrium` namespace, which broke `import jax_fdm.equilibrium.solvers.<module>` whenever isort ordered `.sparse` after `.solvers`. The leading underscore keeps the mapping module-private so it can never leak or collide, independent of import order. Internal only.
- Changed parameter and goal container annotations from `list[X]` to `Sequence[X]` where the callee only iterates: `constrained_fdm`, `Optimizer.problem` and `Optimizer.constraints` (all optimizer variants), `Error.__init__` goals, `ParameterManager.__init__`, and `collect_goals`/`collect_constraints`. Sequences are covariant, so `list[EdgeForceDensityParameter]` no longer errors against `list[Parameter]`.
- Changed goal `target` inputs to the `TargetLike` alias (`float | Float[Array, "..."] | Sequence[float] | Sequence[Sequence[float]]`), matching what the setters' `jnp.asarray` + reshape accepts at runtime; plain lists and tuples now type-check as targets, while COMPAS geometry objects remain deliberately excluded — convert them to plain lists at the call site. `vector` parameters on the angle/normal/tangent goals and the edge angle constraint, and the `polygon` parameter of the curvature constraints, accept float sequences the same way. Datastructure `keys` parameters that are only iterated (`edges_forcedensities` and the other edge getters, vertex/node coordinate and support families) widened from `list`/`Sequence` to `Iterable`, so `mesh.edges()`-style iterators pass directly. The `load` vector parameters widened the same way, to what each setter actually consumes: the single-element setters (`vertex_load`, `node_load`, `edge_load`) accept any `Iterable[float]` since the vector is unpacked once, while the many-element setters (`vertices_loads`, `nodes_loads`, `edges_loads`) require a re-iterable `Sequence[float]` because the same vector is re-read per element — a one-shot iterator there would silently load only the first element. Also fixed the lying `edges_forcedensities(q: list[float])` annotation: the runtime broadcast accepts one scalar, so it is `q: float | None` now.
- Changed the minimum supported Python to 3.11, from 3.10. The 3.10 floor was already notional: the current releases of JAX, jaxtyping, lineax, optimistix and NumPy all require 3.11+, so a 3.10 environment could only resolve stale versions of half the dependency stack. The bump lets the typed signatures use `typing.Self` from the standard library instead of adding a `typing_extensions` dependency. This is a breaking change for 3.10 environments.
- Changed the sparse structures (`GraphSparse` and its subclasses) to keep their edge-node connectivity matrix in JAX sparse format instead of densifying it after a sparse build. The densification worked around a historical JAX bug in reverse-mode gradients through a module-held sparse matrix (`TypeError: ... not 'Zero'`), which is fixed in current JAX; a regression test in `tests/test_sparse_interop.py` guards the fixed behavior and sentinel tests track the sparse ops that remain unsupported. The dense connectivity was both the memory ceiling and a solve-path drag for large structures: at 7320 edges it weighed 218 MB and, kept sparse, the end-to-end sparse-model loss evaluates ~2x and its gradient ~4x faster on CPU. The tributary-load helpers stopped consuming connectivity rows so both dense and sparse structures flow through one code path: `nodes_tributary_edges_load` scatter-adds half-edge loads via `edges_indexed` (equivalent to the old `0.5 * |C|.T @ P` formulation), and the face-load pipeline vmaps over a new `edges_faces_indexed` index array (each edge's up-to-two incident face indices, padded with `-1`) instead of over matrix rows. Dense-vs-sparse equilibrium parity under all three load types is pinned by `tests/test_sparse_parity.py`. This is a breaking change for code reading `structure.connectivity` on sparse structures and expecting a dense array; call `.todense()` to recover one.
- Changed the sparse structures to keep their remaining matrices in JAX sparse format too: `adjacency` on `GraphSparse`, and `connectivity_faces_vertices` and `connectivity_edges_faces` on `MeshSparse`, each an O(n^2) dense array before. The edge-face incidence now assembles directly from the `edges_faces_indexed` index array instead of a dense fill. This is a breaking change for code reading the three matrices off sparse structures and expecting dense arrays; call `.todense()` to recover one.
- Changed the neighborhood goals that consumed rows or columns of a structure matrix to whole-matrix products, which work on either storage format and vectorize better than the previous per-element `vmap`: `NetworkSmoothGoal` and `MeshSmoothGoal` compute all neighbor centroids with adjacency products (the per-node fairness helpers became the array-wide `nodes_nbrs_fairness` and `vertices_nbrs_fairness`), `MeshXYZFaceLaplacianGoal` averages incident face centroids with transpose products of the row-normalized face-vertex connectivity, and `VertexNormalAngleGoal` masks incident faces by comparing the face topology to the vertex index instead of slicing a connectivity column, dropping its cached matrix entirely. All rewritten predictions are bit-identical to the previous implementation on a fixed random state, and dense-sparse goal parity is pinned in `tests/test_sparse_parity.py`. This is a breaking change for callers of the removed per-element fairness helpers.
...
- Changed `face_planarity` to divide the polygon planarity by the face's edge count, turning the per-face score from the sum of absolute normal-edge cosines into their mean. The sum let a face with more edges score higher than a quad for the same per-edge deviation — a corrugated hexagon roughly doubled a comparably corrugated quad — so ngon-heavy regions dominated the `MeshPlanarityGoal` average on mixed-degree meshes. The valid-vertex count already computed for the triangle dispatch is the exact denominator, since a closed polygon has as many edges as vertices and pad edges contribute zero cosine to the sum and nothing to the count. Regression tests in `tests/test_planarity.py` pin the per-edge mean and the padding invariance of values and gradients. This is a breaking change for planarity magnitudes: on a pure-quad mesh the goal's prediction is exactly a quarter of its previous value, so tuned planarity weights may need rescaling.
- Cleaned up the type-checker suppressions and lint configuration surfaced by a pyright audit, with no runtime effect on the paths below. The ruff `notebooks` exclusion was anchored to the repo-root `/notebooks` so it no longer also skipped the `visualization/notebooks` source subpackage, which had been silently un-linted; the newly covered files were brought to zero diagnostics. Removed the `# pyright: ignore` pragmas that no longer suppressed anything: the sparse-override pragmas on the `GraphSparse` and `MeshSparse` connectivity fields and builders (`reportIncompatibleVariableOverride`/`reportIncompatibleMethodOverride` are off in basic mode), the `reportOptionalOperand` pragmas around the sparse transpose in `models` and the mesh Laplacian goal (the `.T` property infers as `None` on the sparse union, so the runtime-identical `.transpose()` method, which is typed to return the matrix, replaces it), the two `reportRedeclaration` pragmas on the nested `eval_fun_and_grad` closures in `gradient_descent` (now annotated `(ParamVector) -> tuple[Any, Any]`), and the `reportArgumentType` pragmas at the `jnp.flatnonzero` size boundary in `structures` (replaced by an eager `int()` on the concrete free/support count) and the `structure_from_datastructure` factory dispatch (each factory is now called inside its own `isinstance` branch so the narrowed type flows through). Also normalized 111 `# noqa F403` directives to the parseable `# noqa: F403` form. Along the way three annotations that disagreed with the runtime were corrected: `EquilibriumStructureSparse.connectivity_free`/`connectivity_fixed` are declared `Float[BCOO, ...]` where they were inheriting a dense `Float[Array, ...]` from the base, `IPOPT.hvp` returns the `(parameters, parameters)` constraint Hessian rather than the `(parameters,)` it claimed, and `VertexTangentAngleGoal.prediction` returns the scalar `Float[Array, ""]` it computes, matching its `VertexNormalAngleGoal` parent, rather than `Float[Array, "1"]`.
- Changed the mesh goals to speak mesh vocabulary in their coordinate-array annotations. The `xyz` parameters of the face and vertex helpers in `goals/mesh/area.py`, `goals/mesh/planarity.py` and `goals/vertex/normal.py` were annotated `Float[Array, "nodes 3"]` while the same functions already named their face index and coordinate returns with the `"vertices"` axis, so each helper contradicted itself. They now read `Float[Array, "vertices 3"]`, and `MeshSmoothGoal.indices_free` reads `Int[Array, "vertices_free"]` in place of `"nodes_free"`. The `EquilibriumState.xyz` field keeps its `"nodes 3"` axis, since it is the shared node-and-vertex abstraction the goals read from. Annotation-only, no runtime effect, and the jaxtyping axis names are documentation here rather than checked shapes.
- Changed `Optimizer.solve` and `GradientFreeOptimizer.solve` to return a jax array. Both are annotated `Float[Array, "parameters"]` but returned the SciPy result vector `res_q.x`, which is NumPy, so the value crossed back into the differentiable pipeline mistyped. Each now returns `jnp.asarray(res_q.x)`, matching the signature and the array kind every downstream caller expects.
- Removed the `from __future__ import print_function` relic from the top-level `jax_fdm/__init__.py`, a no-op since Python 3, and added the missing `-> None` return annotation to the seventeen `__init__` methods across `equilibrium`, `goals`, and `optimization` that still lacked one, so every constructor in the package now declares its return type.

### Fixed

- Fixed `colinearity_points` in `geometry` overcounting its energy through a shape mismatch. The per-edge squared lengths carry a trailing unit axis, so averaging consecutive lengths into the `lbar` normalizer broadcast the `(interior, 1)` denominator against the `(interior,)` numerator and formed an `(interior, interior)` matrix whose off-diagonal cross terms roughly doubled the summed energy. Raveling the averaged lengths back to `(interior,)` restores the intended per-interior-vertex division. `NodesColinearGoal` and `VerticesColinearGoal` read this energy, so their optimization landscapes shift by the corrected scale. The `test_colinear_load_optimization_flattens_arch` fixture, which pinned the pre-fix convergence to `atol=1e-9`, was re-baselined to `atol=1e-8`: the arch still flattens (final loss ~2e-19).
- Fixed a `NameError` in `gradient_descent` when `maxiter=0`. The loop variable `k`, read after the loop to report the iteration count, was never bound when the loop body did not run; it is now initialized to `-1` before the loop.
- Fixed `connectivity_matrix` and `adjacency_matrix` in `equilibrium/structures/` undersizing their node axis when the highest-indexed node touches no edge. Both inferred the node count from the largest index present in the edges, so an isolated top node dropped a column (and, for the adjacency, a row). Both now take an optional `num_nodes` argument, and the `Graph` builders pass `self.num_nodes`, so the matrices always span every node and their `"nodes"` axis annotations are truthful.

- Fixed `FaceRectangularGoal` leaking predictions across collected goals: it cached only its own faces' corner indices at init and summed the rectangularity measure over every cached row in each vmapped prediction call, so when two of these goals were vectorized into one collection, every element received the sum over both faces instead of its own measure. The prediction now gathers one face through the cached face topology per element index, matching the per-element contract of the collection machinery. A regression test in `tests/test_goal_face_rectangle.py` pins collection-vs-individual parity on faces that measure differently.
- Fixed the slow construction of equilibrium structures, which took seconds for meshes beyond a thousand edges and made rebuild-heavy workflows sluggish. Four hot spots, all Python-loop-shaped, accounted for the cost; a 1860-edge mesh structure now builds in ~0.06 s instead of ~5.5 s, and the speedup grows with mesh size since the worst loop was quadratic. Equinox module overhead was measured and ruled out (~0.2 ms per structure). The fixes: `Mesh._edges_faces` now builds a halfedge-to-face dictionary in one pass over the faces and looks each edge up, instead of rescanning every face per edge; `EquilibriumStructureSparse._get_sparse_diag_indices` locates the stiffness diagonal with one vectorized NumPy comparison instead of a per-row `jnp.where` loop, each of which paid a device dispatch; `connectivity_matrix`, `adjacency_matrix` and `face_matrix` convert their input to NumPy once and assemble COO triplets vectorized, instead of iterating a JAX array element-wise at one device sync per element. With the vectorized assembly, rebuilding `GraphSparse.connectivity_scipy` on each of its three accesses per init costs fractions of a millisecond, so the property stays uncached and the module stays immutable. `EquilibriumStructure._indices_freefixed` also swapped its `.item()`-per-node dictionary sort for `np.argsort`, since the argsort of a permutation is its inverse. All rebuilt matrices are bit-identical to the previous implementation.
